...
A few of those modules import the hooks' other packages in turn: `lib.ipc` reads the
client's record through `core.host`, which finds it in `hosts`, to locate the client's MCP
configuration, and `lib.read_model`, which setup's key check uses, imports `lib.ipc`. A
caller that asks for such a module passes `host=True`, and `core` and `hosts` are loaded
the same way, from their files, since they are common names too. The status line and the
project header need `lib` alone, and do not depend on the other two being present.
"""
...
def hooks_lib(*names: str, host: bool = False) -> tuple:
    """The modules `lib.<name>` of the vendored hooks, in the order named.

    `host=True` also loads the hooks' `core` and `hosts` packages, which `lib.ipc` and
    everything that imports it need.
    """
    for package in ("core", "hosts", "lib") if host else ("lib",):
        _vendored(package)
